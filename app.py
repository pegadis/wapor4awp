...
def update_graph(click_data, dropdown_value, download_clicks, line_chart_figure):
    ...
    map_fig.update_layout(
        # Remove the sendDataToCloud, select2d, lasso2d, and resetScale2d buttons
        modebar=dict(
            orientation='h',
            bgcolor='rgba(0,0,0,0)',  # make the modebar transparent
            activecolor='#FFFFFF',
            # Remove the buttons we don't need
            remove=['sendDataToCloud', 'select2d', 'lasso2d', 'resetScale2d', 'toggleSpikelines']
        )
    )

    if click_data is not None:
        line_fig = px.line(filtered_df, x='year', y=column_names, line_group='country_name',
